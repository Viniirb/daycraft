...
export default function CreateEventModal({ isOpen, onClose, selectedInfo }: Props) {
  ...
  const handleSaveEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !selectedInfo || !auth.currentUser) return;
    ...
    try {
      ...
      // Aguarda um pequeno delay para garantir que o Firebase atualizou
                ...
                <form onSubmit={handleSaveEvent} className="mt-6 space-y-4">
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    <button
                      type="button"
                      onClick={onClose}
                      className="rounded bg-gray-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-gray-500"
                    >
                      Cancelar
                    </button>
                    <button
